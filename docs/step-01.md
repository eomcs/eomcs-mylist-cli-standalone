...
프로그래밍의 시작은 **프로젝트 폴더** 를 만드는 것이다.

`프로그래밍`은 **컴퓨터가 해야 할 일을 명령어로 작성하는 것** 을 가르키는 말이다. 프로그래밍 중에 생성되는 파일을 두는 폴더가 프로젝트 폴더다.
...
차츰 _그레이들_ 로 이동하는 추세다.
특히 _안드로이드 앱 개발_ 쪽에서는 기본 빌드 도구로 _그레이들_ 을 사용한다.
